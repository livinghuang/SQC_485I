/*
  SQC_485I RS485-to-LoRaWAN Example
...
static void prepareTxFrame(uint8_t port)
{
  ...
  if (rs485_received != "")
  {
    Serial.println("RS485 Received data: " + rs485_received);
    appDataSize = rs485_received.length();
    if (appDataSize > LORAWAN_APP_DATA_MAX_SIZE)
    {
      appDataSize = LORAWAN_APP_DATA_MAX_SIZE; // Limit to max size
    }
    rs485_received.getBytes(appData, appDataSize);
  }
  else
  {
    appDataSize = 4;
...
void beginRS485()
{
  ...
  sendRS485("Hello, I'm Rs485 to LoRaWAN!"); // Initial message
